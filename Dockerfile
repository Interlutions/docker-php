...
RUN apk add --no-cache --virtual .build-deps $PHPIZE_DEPS && \
    ...
    docker-php-ext-install -j${NPROC} bz2 dom exif fileinfo hash iconv mcrypt intl opcache pcntl pdo pdo_mysql pdo_sqlite readline session simplexml xml xsl zip gd soap && \
